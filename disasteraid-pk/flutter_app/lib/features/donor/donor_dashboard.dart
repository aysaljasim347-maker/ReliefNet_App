...
class _DonorHomeTabState extends State<DonorHomeTab> {
  ...
  Future<void> _loadStats() async {
    ...
    try {
      ...
      final results = await Future.wait([campaignFuture, inKindFuture]);
      final campaigns = results[0] as List;
      final inKindRes = results[1] as dynamic;

      final donations = SafeDataHandler.extractList(inKindRes.data);
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Dashboard')),
        body: ErrorState(message: _error!, onRetry: _loadStats),
      );
    }
